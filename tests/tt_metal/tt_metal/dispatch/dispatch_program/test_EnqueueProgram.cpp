...
INSTANTIATE_TEST_SUITE_P(CommandQueue, CQMultiDeviceOnFabricFixture, CQFabricConfigsToTest);

INSTANTIATE_TEST_SUITE_P(MultiCommandQueue, MultiCQMultiDeviceOnFabricFixture, CQFabricConfigsToTest);

TEST_P(CQMultiDeviceOnFabricFixture, TensixTestBasicDispatchFunctions) {
    for (auto& device : devices_) {
        local_test_functions::test_basic_dispatch_functions(device, 0);
    }
}

TEST_P(MultiCQMultiDeviceOnFabricFixture, TensixTestBasicDispatchFunctions) {
    for (auto& device : devices_) {
