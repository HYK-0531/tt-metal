...
namespace unit_tests::dm::matmul_interleaved {

constexpr uint32_t START_ID = 550;

// Test config, i.e. test parameters
struct MatmulInterleavedConfig {
    uint32_t test_id = 0;
};
...
/// @return
bool run_dm(IDevice* device, const MatmulInterleavedConfig& test_config) {
    // Program
    ...
    auto risc0_kernel = CreateKernel(
        program,
        "tests/tt_metal/tt_metal/data_movement/matmul_interleaved/kernels/2d_matmul_in0_interleaved.cpp",
        matmul_cores,
        DataMovementConfig{
...
TEST_F(DeviceFixture, TensixDataMovementMatmulInterleaved) {
    // Parameters
    uint32_t test_id = unit_tests::dm::matmul_interleaved::START_ID + 0;

    // Test config
    unit_tests::dm::matmul_interleaved::MatmulInterleavedConfig test_config = {.test_id = test_id};
