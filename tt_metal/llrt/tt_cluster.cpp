...
inline std::string get_soc_description_file(
    const tt::ARCH& arch, tt::TargetDevice target_device, const llrt::RunTimeOptions& rtoptions, [[maybe_unused]] const std::string& output_dir = "") {
    // Ability to skip this runtime opt, since trimmed SOC desc limits which DRAM channels are available.
    std::string path;
    if (rtoptions.is_root_dir_specified()) {
        path = rtoptions.get_root_dir();
    } else {
        path = "./";
    }
    if (path.back() != '/') {
...
void Cluster::open_driver(const bool &skip_driver_allocs) {
    ...
    if (this->target_type_ == TargetDevice::Silicon) {
        ...
        device_driver = std::make_unique<tt::umd::Cluster>(tt::umd::ClusterOptions{
            ...
            .sdesc_path = get_soc_description_file(this->arch_, this->target_type_, this->rtoptions_),
