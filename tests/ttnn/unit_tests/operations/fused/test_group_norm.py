...
@pytest.mark.parametrize(
    "N, C, H, W, num_groups, num_out_blocks, cores_x, cores_y",
    [
        # num_groups is factor of C
        # num_out_blocks cores_x is factor of num_groups
        [1, 640, 128, 128, 32, 3, 8, 4],
        # [1, 960, 128, 128, 32, 6, 8, 2],
        # [1, 128, 1024, 1024, 32, 128, 8, 4],
...
def test_group_norm_bh_grid(device, N, C, H, W, num_groups, num_out_blocks, cores_x, cores_y):
    torch.manual_seed(0)
    compute_grid_size = device.compute_with_storage_grid_size()
    print(f"compute_grid_size: {compute_grid_size}")

    if compute_grid_size.x < cores_x:
        pytest.skip(f"Skipping test because compute grid size {compute_grid_size} is less than cores_x {cores_x}")
    if compute_grid_size.y < cores_y:
        pytest.skip(f"Skipping test because compute grid size {compute_grid_size} is less than cores_y {cores_y}")
