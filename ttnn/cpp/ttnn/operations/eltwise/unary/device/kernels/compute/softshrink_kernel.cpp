...
void MAIN {
    ...
    constexpr auto cb_tmp1 = tt::CBIndex::c_3;
    init_sfpu(cb_input, cb_output);

    // a+λ⋅1(a+λ<0)+ ((a-λ)⋅1(a−λ>0))
    for (uint32_t block_index = 0; block_index < per_core_block_cnt; block_index++) {
        ...
        for (uint32_t tile_index = 0; tile_index < per_core_block_dim; ++tile_index) {
            ...
            binary_dest_reuse_tiles_init<EltwiseBinaryType::ELWADD, EltwiseBinaryReuseDestType::DEST_TO_SRCA>(cb_tmp1);
            binary_dest_reuse_tiles<EltwiseBinaryType::ELWADD, EltwiseBinaryReuseDestType::DEST_TO_SRCA>(cb_tmp1, 0, 0);
