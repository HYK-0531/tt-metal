...
@pytest.mark.parametrize(
    ...
    ((10, 100, ttnn.bfloat8_b, ttnn.bfloat8_b),),
)
...
def test_mobilenetv2_imagenet_demo_data_parallel(
    mesh_device,
    batch_size_per_device,
    iterations,
    imagenet_label_dict,
    ...
    with torch.no_grad():
        ...
        for iter in tqdm(range(iterations), desc="Preparing images"):
            inputs, labels = get_batch(data_loader, image_processor)
            # inputs = torch.permute(inputs, (0, 2, 3, 1))  # NCHW -> NHWC
            input_tensors_all.append(inputs)
            input_labels_all.append(labels)
        logger.info("Processed ImageNet-1k validation Dataset")
        ...
        for iter in range(iterations):
            ...
            tt_inputs_host = ttnn.from_torch(
                ...
                mesh_mapper=mobilenetv2_trace_2cq.test_infra.inputs_mesh_mapper,
            )
            print("Before trace")
            tt_inputs_host = ttnn.pad(tt_inputs_host, [1, 1, n * h * w, 16], [0, 0, 0, 0], 0)
            output = mobilenetv2_trace_2cq.execute_mobilenetv2_trace_2cqs_inference(tt_inputs_host)
