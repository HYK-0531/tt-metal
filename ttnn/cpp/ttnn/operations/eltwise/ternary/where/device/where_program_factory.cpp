    ...
    for (uint32_t i = 0, start_tile_id = 0; i < num_cores_total; i++) {
        ...
        if (variant == WhereVariant::TTS) {
            std::array compute_runtime_args = {
                num_tiles_per_core, std::bit_cast<uint32_t>(operation_attributes.value_false_scalar.value())};
            handle_args(program, compute_kernel_id, core, compute_runtime_args);
        } else if (variant == WhereVariant::TST) {
            std::array compute_runtime_args = {
                num_tiles_per_core, std::bit_cast<uint32_t>(operation_attributes.value_true_scalar.value())};
            handle_args(program, compute_kernel_id, core, compute_runtime_args);
        } else {
...
WhereDeviceOperation::WhereProgramFactory::cached_program_t WhereDeviceOperation::WhereProgramFactory::create(
    ...
    if (variant == WhereVariant::TTS) {
        float false_scalar = operation_attributes.value_false_scalar.value();
        std::cout << "false_scalar: " << false_scalar << std::endl;
        compute_kernel_args = {
            num_tiles_per_cycle, std::bit_cast<uint32_t>(operation_attributes.value_false_scalar.value())};
    } else if (variant == WhereVariant::TST) {
        compute_kernel_args = {
            num_tiles_per_cycle, std::bit_cast<uint32_t>(operation_attributes.value_true_scalar.value())};
    } else {
