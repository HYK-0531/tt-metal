...
For a kernel running on data movement core 0 of Tensix core (0, 0), the compiled binary is first stored on the Tensix's shared SRAM (referred to as L1). During execution, code is fetched from shared SRAM and cached in a per-RISC-V private instruction cache of 0.5~2KiB (128~512 instructions) depending on the core type. This reduces SRAM access pressure. The RISC-V cores also contain private memory for local variables and the stack. All RISC-V cores have their own private memory and shared SRAM mapped at the same address. Shared SRAM is accessible to all cores at all times.

See the `Baby RISC-V <https://github.com/tenstorrent/tt-isa-documentation/blob/main/WormholeB0/TensixTile/BabyRISCV/README.md>`_ documentation for more details on the RISC-V address space.
