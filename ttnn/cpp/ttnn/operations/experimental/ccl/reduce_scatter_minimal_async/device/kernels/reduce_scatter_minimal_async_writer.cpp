...
void kernel_main() {
    ...
    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": my_chip_id " << my_chip_id << ENDL();
    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": out_ready_sem " << (uint32_t)out_ready_sem
           << " value " << *reinterpret_cast<volatile tt_l1_ptr uint32_t*>(out_ready_sem) << ENDL();
    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": batch_ready_sem " << (uint32_t)batch_ready_sem
           << " value " << *reinterpret_cast<volatile tt_l1_ptr uint32_t*>(batch_ready_sem) << ENDL();
    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": term_ready_sem " << (uint32_t)term_ready_sem
           << " value " << *reinterpret_cast<volatile tt_l1_ptr uint32_t*>(term_ready_sem) << ENDL();

    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": local_fabric_mux_status_address "
           << (uint32_t)local_fabric_mux_status_address << " value "
           << *reinterpret_cast<volatile tt_l1_ptr uint32_t*>(local_fabric_mux_status_address) << ENDL();
    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": local_flow_control_address "
           << (uint32_t)local_flow_control_address << " value "
           << *reinterpret_cast<volatile tt_l1_ptr uint32_t*>(local_flow_control_address) << ENDL();
    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": local_teardown_address "
           << (uint32_t)local_teardown_address << " value "
           << *reinterpret_cast<volatile tt_l1_ptr uint32_t*>(local_teardown_address) << ENDL();
    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": local_buffer_index_address "
           << (uint32_t)local_buffer_index_address << " value "
           << *reinterpret_cast<volatile tt_l1_ptr uint32_t*>(local_buffer_index_address) << ENDL();

    DPRINT << "WRITER: fabric_mux_channel_base_address " << (uint32_t)fabric_mux_channel_base_address << ENDL();

    DPRINT << "WRITER: fabric_mux_buffer_index_address " << (uint32_t)fabric_mux_buffer_index_address << ENDL();
    ...
    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": fabric mux waiting for ready" << ENDL();
    tt::tt_fabric::wait_for_fabric_endpoint_ready(
        fabric_mux_x, fabric_mux_y, fabric_mux_status_address, local_fabric_mux_status_address);

    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": fabric mux ready" << ENDL();
    ...
    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": fabric client connecting" << ENDL();
    tt::tt_fabric::fabric_client_connect(mux_connection_handle);

    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": fabric client connected" << ENDL();
    ...
    for (uint32_t b = 0; b < num_batches; b++) {
        ...
        for (uint32_t i = 0; i < ring_size; ++i) {
            DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": iteration " << i << ENDL();
            ...
            if (i < (ring_size - 1)) {
                ...
                // DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": tiles_read " << tiles_read << "
                ...
                while (tiles_read < tiles_to_read) {
                    ...
                    // DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": waited for cb_output_id with
                    ...
                    for (uint32_t j = 0; j < num_pages_to_read; j += contig_pages_advanced) {
                        ...
                        // DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": wrote to noc with
                        // tile_read:" << tiles_read << ENDL(); Note: Must flush write for correctness
    ...
    // TODO: zero out semaphores
    noc_async_write_barrier();
    DPRINT << "WRITER " << (direction ? "forward" : "backward") << ": done" << ENDL();
}
