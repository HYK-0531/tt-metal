...
WhereDeviceOperation::WhereProgramFactory::cached_program_t WhereDeviceOperation::WhereProgramFactory::create(
    ...
    auto predicate_data_format = datatype_to_dataformat_converter(
        (predicate_tensor.dtype() == DataType::BFLOAT16 || predicate_tensor.dtype() == DataType::BFLOAT8_B)
            ? DataType::UINT16
            : predicate_tensor.dtype());
    auto value_true_data_format = datatype_to_dataformat_converter(
        (value_true_tensor.dtype() == DataType::BFLOAT16 || value_true_tensor.dtype() == DataType::BFLOAT8_B)
            ? DataType::UINT16
            : value_true_tensor.dtype());
    auto value_false_data_format = datatype_to_dataformat_converter(
        (value_false_tensor.dtype() == DataType::BFLOAT16 || value_false_tensor.dtype() == DataType::BFLOAT8_B)
            ? DataType::UINT16
            : value_false_tensor.dtype());
    auto output_data_format = datatype_to_dataformat_converter(
        (output.dtype() == DataType::BFLOAT16 || output.dtype() == DataType::BFLOAT8_B) ? DataType::UINT16
                                                                                        : output.dtype());
