...
inline void GlobalAllocator::allocate_resources(TestConfig& test_config) {
    ...
    // Allocate all patterns by link groups
    for (auto& [link_id, group_patterns] : link_groups) {
        allocate_link_group(group_patterns, link_id);
    }
}
...
inline void GlobalAllocator::allocate_multicast_patterns_uniform(
    std::vector<std::tuple<SenderConfig*, TrafficPatternConfig*, DestinationConfig*>>& multicast_patterns,
    uint32_t link_id) {
    // Collect all unique destinations from this multicast group
    std::set<FabricNodeId> unique_destinations;
    for (auto& [sender_ptr, pattern_ptr, dest_ptr] : multicast_patterns) {
        ...
        for (const auto& dst_node : dst_node_ids) {
            unique_destinations.insert(dst_node);
        }
    }
