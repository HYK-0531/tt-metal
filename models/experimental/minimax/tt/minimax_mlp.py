...
class TTMiniMaxM1MLP(nn.Module):
    def __init__(self, config, state_dict, base_address, device=None):
        ...
        self.tt_gate_proj_weight = ttnn.from_torch(state_dict[f"{base_address}.gate_proj.weight"], self.device)
        ...
        self.tt_up_proj_weight = ttnn.from_torch(state_dict[f"{base_address}.up_proj.weight"], self.device)
        ...
        self.tt_down_proj_weight = ttnn.from_torch(state_dict[f"{base_address}.down_proj.weight"], self.device)
...
class TTMiniMaxM1BlockSparseTop2MLP(nn.Module):
    def __init__(self, config, state_dict, base_address, device=None):
        ...
        self.tt_w1_weight = ttnn.from_torch(state_dict[f"{base_address}.w1.weight"], self.device)
        self.tt_w1 = TTLinear(self.tt_w1_weight.padded_shape[-1], self.tt_w1_weight.padded_shape[-2], self.tt_w1_weight)

        self.tt_w2_weight = ttnn.from_torch(state_dict[f"{base_address}.w2.weight"], self.device)
        self.tt_w2 = TTLinear(self.tt_w2_weight.padded_shape[-1], self.tt_w2_weight.padded_shape[-2], self.tt_w2_weight)

        self.tt_w3_weight = ttnn.from_torch(state_dict[f"{base_address}.w3.weight"], self.device)
