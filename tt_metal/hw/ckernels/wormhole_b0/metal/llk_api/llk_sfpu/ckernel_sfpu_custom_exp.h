...
// - exp_21f_alt: accurate version (minor optimization tricks [untested])
...
enum ExpVariant { EXP_ACCURATE_BASE, EXP_ACCURATE_HYBRID0, EXP_APPROX, EXP_APPROX_21F };
    ...
    if constexpr (FAST_APPROX && APPROXIMATION_MODE) {
        ...
        for (int d = 0; d < iterations; d++) {
            ...
            if constexpr (SELECTED_EXP == ExpVariant::EXP_APPROX_21F) {
                ...
                v_elseif(val < -42) { sfpi::dst_reg[0] = 0.0f; }
                v_else {
                    // * by 1/ln2 and add convert to 7.3 FxP format
                    sfpi::vFloat vConstLn2Recip = sfpi::vConstFloatPrgm0;
                ...
                DPRINT_MATH(DPRINT << "calculate_exponential_approx" << ENDL());
...
inline void _init_exponential_() {
    if constexpr (FAST_APPROX && APPROXIMATION_MODE) {
        ...
        SELECTED_EXP == ExpVariant::EXP_ACCURATE_BASE || SELECTED_EXP == ExpVariant::EXP_ACCURATE_HYBRID0) {
        sfpi::vConstFloatPrgm0 = 1.442695f;  // ln2_recip
        sfpi::vConstFloatPrgm1 = 2.0f;
