...
from .fun_conv2d import TtConv2dParameters
        ...
        *,
        ...
        return cls(
            norm1=TtGroupNormParameters.from_torch(
                resnet_block.norm1.get_state(),
                device=device,
                num_channels=resnet_block.norm1.num_channels,
                num_groups=resnet_block.norm1.num_groups,
            ),
            norm1=TtGroupNormParameters.from_torch(
                resnet_block.norm2.get_state(),
                device=device,
                num_channels=resnet_block.norm2.num_channels,
                num_groups=resnet_block.norm2.num_groups,
            ),
            conv1=TtConv2dParameters.from_torch(resnet_block.conv1.get_state(), dtype=dtype, device=device),
            conv2=TtConv2dParameters.from_torch(resnet_block.conv2.get_state(), dtype=dtype, device=device),
            conv_shortcut=TtConv2dParameters.from_torch(
                substate(resnet_block.get_state(), "conv_shortcut"), dtype=dtype, device=device
            )
            if has_substate(resnet_block.get_state(), "conv_shortcut")
            else None,
        )
...
def sd_vae_decode(
    ...
    # TODO: Add group norm to normalizations and test
    # Main modules: Conv2D, unet_mid, up_decoder
    # conv_in
    # Mid block
    # up block for loop
    # . ...
    # conv norm
    # activation. How do we do in place?
    # conv out. Return
    pass


def unet_mid_block2d(params):
    pass


def up_decoder_block(params):
    pass
