...
TEST_F(DeviceFixture, TensixMaxCommonRuntimeArgsCompute) {
    auto& rtoptions = tt_metal::MetalContext::instance().rtoptions();
    uint32_t metal_max_runtime_args = 6144;
    for (size_t max_runtime_args = 256; max_runtime_args < metal_max_runtime_args / 2;
        ...
        for (unsigned int id = 0; id < num_devices_; id++) {
            ...
            tt_metal::detail::LaunchProgram(this->devices_.at(id), program);
            unit_tests::runtime_args::verify_results(
                true, this->devices_.at(id), program, {}, common_runtime_args);
        }
    }
