...
public:
    void validate_local_mesh_id_and_host_rank() {
        ...
        TT_FATAL(
            local_mesh_id_.size() and local_host_rank_.size(),
    ...
    uint32_t get_local_mesh_id() const { return std::stoi(local_mesh_id_); }

    uint32_t get_local_host_rank() const { return std::stoi(local_host_rank_); }

private:
    inline static std::string local_mesh_id_ = "";
    inline static std::string local_host_rank_ = "";
    inline static bool bindings_initialized = false;
};
