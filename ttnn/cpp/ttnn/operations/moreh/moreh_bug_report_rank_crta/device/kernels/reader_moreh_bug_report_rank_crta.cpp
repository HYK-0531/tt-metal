...
void kernel_main() {
    ...
    constexpr uint32_t input_cta_base = 2;
    constexpr uint32_t input_crta_base = 2;
    auto input_args = nd_sharding::make_args<input_cta_base, input_crta_base>();
    constexpr uint32_t input_cta_skip = input_args.compile_time_args_skip();
    uint32_t input_crta_skip = input_args.runtime_args_skip();

    constexpr uint32_t other_cta_base = input_cta_base + input_cta_skip;
    uint32_t other_crta_base = input_crta_base + input_crta_skip;
    auto other_args = nd_sharding::make_args<other_cta_base>(other_crta_base);
    ...
    auto input_sharded_accessor =
        nd_sharding::make_sharded_accessor_from_args(input_args, input_addr, cb_input_tile_size);
    auto other_sharded_accessor =
        nd_sharding::make_sharded_accessor_from_args(other_args, other_addr, cb_other_tile_size);
    ...
    for (uint32_t i = start_id; i < end_id; ++i) {
        ...
        input_sharded_accessor.noc_async_read_page(i, cb_input_write_ptr);

        uint32_t cb_other_write_ptr = get_write_ptr(cb_other);
        other_sharded_accessor.noc_async_read_page(i, cb_other_write_ptr);
