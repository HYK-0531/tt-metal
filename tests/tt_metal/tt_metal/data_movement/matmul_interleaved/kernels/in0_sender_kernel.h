...
void in0_sender_run(
...

    {
        ...
        for (uint32_t h = 0; h < num_subblocks_k_dim; h += subblock_k_dim) {
            // noc_semaphore_wait(in0_mcast_sender_semaphore_addr_ptr, num_of_receivers_x);
            noc_semaphore_set(in0_mcast_sender_semaphore_addr_ptr, 0);
            noc_async_write_multicast(
                in0_read_address, in0_multicast_start_address, in0_block_size_bytes, num_of_receivers_x - 1);

            // This needs snoop bit enabled
            noc_semaphore_set_multicast(
                in0_mcast_sender_semaphore_valid_addr, in0_mcast_receiver_semaphore_noc_addr, num_of_receivers_x);
