...
def get_rank_environment(binding: RankBinding, config: TTRunConfig) -> Dict[str, str]:
    ...
    env = {
        ...
        "TT_METAL_CACHE": os.environ.get("TT_METAL_CACHE", DEFAULT_CACHE_DIR_PATTERN.format(
            home=str(Path.home()),
...
def build_mpi_command(
    config: TTRunConfig, program: List[str], mpi_args: Optional[List[str]] = None
) -> List[str]:
...
@click.option("--mpi-args", callback=lambda ctx, param, value: shlex.split(value) if value else None, help="Additional MPI arguments (quoted)")
@click.pass_context
