...
TEST_F(ControlPlaneLocalMeshBinding, NoEnvironmentVariables) {
    ...
    EXPECT_EQ(control_plane->get_local_mesh_id_binding().value(), MeshId{0});
    EXPECT_EQ(control_plane->get_local_host_rank_id_binding().value(), HostRankId{0});
}
...
TEST_P(MeshScopeParameterizedTest, GetPhysicalMeshShape) {
    ...
    EXPECT_EQ(control_plane->get_local_mesh_id_binding().value(), MeshId{0});
    EXPECT_EQ(control_plane->get_local_host_rank_id_binding().value(), params.host_rank);
...
Graph: []
    ...
    auto mesh_binding = control_plane->get_local_mesh_id_binding().value();
    auto host_binding = control_plane->get_local_host_rank_id_binding().value();
