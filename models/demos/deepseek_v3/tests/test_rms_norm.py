...
from models.demos.deepseek_v3.tt.rms_norm.decoder_rms_norm import DecoderRMSNorm
from models.demos.deepseek_v3.tt.rms_norm.kq_rms_norm import KQRMSNorm
...
def test_forward_pass_for_decoder_norm(
    ...
    weight_config = DecoderRMSNorm.convert_weights(hf_config, hf_state_dict, tmpdir, mesh_row)

    # Generate the model config
    if mode == "prefill":
        model_config = DecoderRMSNorm.prefill_model_config(hf_config, mesh_row)
    else:
        model_config = DecoderRMSNorm.decode_model_config(hf_config, mesh_row)

    # Generate a new model state
    model_state = DecoderRMSNorm.create_state(hf_config, mesh_row)
    ...
    if mode == "prefill":
        ...
        memory_config = ttnn.create_sharded_memory_config(
            shape=(
                ...
                ttnn.core.roundup(even_int_div(hf_config.hidden_size, shard_core_grid.num_cores), ttnn.TILE_SIZE),
            ),
    ...
    if mode == "prefill":
        tt_output = DecoderRMSNorm.forward_prefill(tt_input, run_config)
    else:
        tt_output = DecoderRMSNorm.forward_decode(tt_input, run_config)

    tt_output_torch = ttnn.to_torch(
        tt_output,
        mesh_composer=ttnn.ConcatMesh2dToTensor(mesh_row, dims=(-1, -2), mesh_shape=tuple(mesh_row.shape)),
    )
...
def test_rmsnorm_forward_pass_for_kq_norm(
    ...
    # Generate the weight config
    hf_state_dict = reference_model.state_dict()
    weight_config = KQRMSNorm.convert_weights(hf_config, hf_state_dict, tmpdir, mesh_row)

    # Generate the model config
    if mode == "prefill":
        model_config = KQRMSNorm.prefill_model_config(hf_config, mesh_row)
    else:
        model_config = KQRMSNorm.decode_model_config(hf_config, mesh_row)

    # Generate a new model state
    model_state = KQRMSNorm.create_state(hf_config, mesh_row)
    ...
    # Run TTNN forward pass
    if mode == "prefill":
        tt_output = KQRMSNorm.forward_prefill(tt_input, run_config)
    else:
        tt_output = KQRMSNorm.forward_decode(tt_input, run_config)
