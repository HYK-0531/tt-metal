...
class TtAttentionParameters:
    ...
    def from_torch(
        ...
        *,
        ...
        return cls(
            group_norm=TtGroupNormParameters.from_torch(torch_attention.group_norm, device=device, core_grid=core_grid),
            to_q=TtLinearParameters.from_torch(torch_attention.to_q, dtype=dtype, device=device, core_grid=core_grid),
            to_k=TtLinearParameters.from_torch(torch_attention.to_k, dtype=dtype, device=device, core_grid=core_grid),
            to_v=TtLinearParameters.from_torch(torch_attention.to_v, dtype=dtype, device=device, core_grid=core_grid),
            to_out=[
                TtLinearParameters.from_torch(linear_out, dtype=dtype, device=device, core_grid=core_grid)
...
def vae_attention(
    x: ttnn.Tensor, parameters: TtAttentionParameters, parallel_manager: StableDiffusionParallelManager | None
) -> ttnn.Tensor:
